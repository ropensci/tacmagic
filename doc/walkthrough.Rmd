...
This vignette demonstrates the main features of tacmagic. There are two approaches to using the tacmagic package: either by loading participant data individually and using the various functions to analyze it, or via the batch functions to list and analyze data from multiple participants. Here, to illustrate the main features of tacmagic, we will cover the analysis of a single participant, with an explanation of the batch mode at the end.
...
Time-activity curve data is loaded via `load_tac()`, regardless of format, which is specified by the `format` parameter. In addition to time activity curve (tac) data for 1 or more region of interest (ROI), the start and stop times of each frame are required. This information may be in 1 or more files depending on the software that created it. For example, PMOD's .tac files contain all of the information, but the tac .voistat files do not contain start and stop times, but this information could be specified using a .acqtimes file. Support is also available for DFT format, which contains both TAC and volume data.
...
If this is done, the means must be weighted for the relative volumes of the atomic ROIs. If volume information is available, `tac_roi()` provides this functionality.
...
Basic tac plotting is done with `plot_tac`, which accepts tac data from 2 participants (or group means). The ROIs to plot are specified as a vector of ROI names as they appear in the tac object. As the tac object contains time unit information, the plot can conver to desired units, which can be specified with the `time` argument.
...
The above would load the appropriate tac and voistat files, perform the ROI merging specified by `ROI_def` because `roi_m=T`, and would return a list where each element represents a participants, e.g. the first participant would be `my_data$participant1`.
